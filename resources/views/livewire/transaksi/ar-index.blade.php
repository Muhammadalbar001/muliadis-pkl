<div>
    <div class="min-h-screen space-y-6 pb-10 transition-colors duration-300 font-jakarta"
                ...
                <div class="flex flex-wrap sm:flex-nowrap gap-3 items-center w-full xl:w-auto justify-end">
                    ...
                    <div class="relative w-full sm:w-48 group">
                        ...
                        <input wire:model.live.debounce.300ms="search" type="text"
                            class="w-full pl-9 pr-4 py-2.5 rounded-xl border text-[11px] font-bold uppercase tracking-widest focus:ring-2 focus:ring-amber-500/20 transition-all
                        ...
                        dark:bg-slate-900 border-slate-800 bg-white border-slate-200" style="display: none;">
                            ...
                            <label
                                ...
                                <input type="checkbox" value="{{ $c }}" x-model="selected"
                                    class="rounded-full border-slate-500 text-amber-600 focus:ring-amber-500 h-3.5 w-3.5 focus:ring-offset-0 bg-transparent">
                                <span
                                    class="ml-3 text-[10px] font-bold uppercase tracking-tight group-hover:text-amber-400 dark:text-slate-400 text-slate-600">{{ $c }}</span>
                            </label>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <button wire:click="openImportModal"
                        class="flex items-center justify-center gap-2 px-6 py-2.5 bg-amber-600 hover:bg-amber-700 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-amber-600/20 transition-all transform active:scale-95">
                    ...
                    <table class="w-full text-xs text-left border-collapse uppercase">
                        <thead>
                            <tr
                                ...
                                <th class="px-6 py-5 border-r dark:border-white/5 border-slate-100">Tgl Transaksi</th>
                                <th class="px-6 py-5 border-r dark:border-white/5 border-slate-100">No. Faktur</th>
                                ...
                                <th
                                    class="px-6 py-5 border-r dark:border-white/5 border-slate-100 text-right dark:bg-amber-600/10 bg-amber-50/50 text-amber-600">
                                    Nilai Piutang</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y dark:divide-white/5 divide-slate-100">
                            @forelse($ars as $item)
                            <tr class="hover:bg-amber-500/[0.02] transition-colors group">
                                <td
                                    class="px-6 py-4 dark:text-slate-400 text-slate-500 font-bold border-r dark:border-white/5 border-slate-50 italic">
                                    {{ date('d/m/Y', strtotime($item->tgl_penjualan ?? $item->created_at)) }}
                                </td>
                                <td
                                    class="px-6 py-4 font-mono font-black text-amber-600 dark:text-amber-400 border-r dark:border-white/5 border-slate-50">
                                    {{ $item->no_penjualan ?? $item->trans_no }}
                                </td>
                                <td class="px-6 py-4 font-black dark:text-white text-slate-800 border-r dark:border-white/5 border-slate-50 truncate max-w-[250px]"
                                    ...
                                    {{ $item->pelanggan_name ?? $item->nama_pelanggan }}
                                </td>
                                <td
                                ...
                                <td
                                    class="px-6 py-4 text-right font-black dark:text-white text-slate-900 border-r dark:border-white/5 border-slate-50 bg-amber-500/[0.01]">
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-24 text-center opacity-20">
                                    ...
                                    <p class="text-xs font-black tracking-[0.4em]">Belum Ada Data Piutang</p>
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
        ...
        {{-- MODAL UNGGAH --}}
        @if($isImportOpen)
        @include('livewire.partials.import-modal', ['title' => 'Sinkronisasi Data Piutang (AR)', 'color' => 'amber'])
        @endif
            ...
            <div
                ...
                <div
                    ...
                    <h3
                        ...
                        <i class="fas fa-exclamation-triangle"></i> Pengajuan Hapus AR
                    </h3>
