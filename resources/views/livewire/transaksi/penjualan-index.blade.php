<div class="min-h-screen pb-20 font-jakarta bg-slate-50 dark:bg-[#050505] transition-colors duration-300">
    ...
    <div
        ...
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                <div
                    class="p-3 rounded-2xl shadow-lg bg-gradient-to-br from-blue-600 to-indigo-600 text-white flex items-center justify-center ring-4 ring-blue-500/20">
                    <i class="fas fa-shopping-cart text-xl"></i>
                </div>
                <div>
                    <h1 class="text-2xl font-black uppercase leading-none dark:text-white text-slate-800">
                        Manajemen <span class="text-blue-600 dark:text-blue-400">Penjualan</span>
                    </h1>
                    <p
                        class="text-[10px] font-bold uppercase tracking-[0.3em] opacity-60 mt-1 dark:text-slate-400 text-slate-500">
                        Integrasi Omzet & Faktur Toko
                    </p>
                </div>
            </div>

            <div class="flex flex-wrap items-center gap-2">
                {{-- Tombol Impor (Jika ada halamannya) --}}
                <button
                ...
                <button wire:click="create"
                    class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl flex items-center gap-2 shadow-lg shadow-blue-500/30 transition-transform active:scale-95 font-black text-[10px] uppercase tracking-widest">
        ...
        <div
            ...
            <div class="p-6 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02]">
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                    ...
                    {{-- 1. Search Bar (Mempertahankan yang bagus) --}}
                    <div class="md:col-span-4">
                        ...
                        <div class="relative">
                            ...
                            <input wire:model.live.debounce.500ms="search" type="text"
                                class="block w-full pl-11 pr-4 py-3 border border-slate-200 dark:border-white/10 rounded-2xl leading-5 bg-white dark:bg-[#1a1a1a] text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-xs font-bold uppercase tracking-wider shadow-sm transition-all"
                                placeholder="Cari Nota, Toko, Sales...">
                        </div>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select wire:model.live="filter_cabang"
                            class="block w-full px-4 py-3 border border-slate-200 dark:border-white/10 rounded-2xl leading-5 bg-white dark:bg-[#1a1a1a] text-slate-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs font-bold uppercase tracking-wider shadow-sm cursor-pointer transition-all">
                ...
                <table class="w-full text-sm text-left uppercase whitespace-nowrap">
                    <thead>
                        <tr
                            ...
                            <th class="px-6 py-5">Tgl Transaksi</th>
                            <th class="px-6 py-5">No Nota / Dokumen</th>
                            <th class="px-6 py-5">Cabang</th>
                            <th class="px-6 py-5">Salesman</th>
                            <th class="px-6 py-5">Toko / Pelanggan</th>
                            <th class="px-6 py-5 text-right">Grand Total (Omzet)</th>
                            <th class="px-6 py-5 text-center">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 dark:divide-white/5">
                        @forelse($penjualan as $item)
                        <tr class="hover:bg-blue-50 dark:hover:bg-white/[0.02] transition-colors group">
                            ...
                            <td class="px-6 py-4">
                                <span
                                    class="px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest bg-slate-100 text-slate-600 dark:bg-white/5 dark:text-slate-300 border border-slate-200 dark:border-white/10">
                                    {{ $item->cabang }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-xs font-black text-right text-emerald-600 dark:text-emerald-400">
                            ...
                            <td class="px-6 py-4 text-center">
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    <button wire:click="edit({{ $item->id }})"
                                        class="w-8 h-8 rounded-xl bg-white dark:bg-[#1a1a1a] text-blue-600 hover:bg-blue-600 hover:text-white transition-colors border border-slate-200 dark:border-white/10 shadow-sm flex items-center justify-center"
                                        title="Edit Data">
                                        <i class="fas fa-pen text-[10px]"></i>
                                    </button>
                                    <button wire:click="delete({{ $item->id }})"
                                        wire:confirm="HAPUS PERMANEN: Yakin ingin menghapus data penjualan dengan No Nota {{ $item->trans_no }}?"
                                        class="w-8 h-8 rounded-xl bg-white dark:bg-[#1a1a1a] text-rose-600 hover:bg-rose-600 hover:text-white transition-colors border border-slate-200 dark:border-white/10 shadow-sm flex items-center justify-center"
                                        title="Hapus Data">
                                ...
                                @else
                                <span
                                    class="text-[9px] font-black text-slate-400 border border-slate-200 dark:border-white/10 px-2 py-1 rounded-md bg-slate-50 dark:bg-white/5 cursor-not-allowed"
                                    title="Operator tidak diizinkan mengedit/menghapus">
                                    <i class="fas fa-lock text-rose-400 mr-1"></i> Terkunci
                                </span>
                                @endif
                            </td>
                        </tr>
    ...
    {{-- ==================================================== --}}
    {{-- MODAL FORM CRUD TIPE A (PENJUALAN)                   --}}
    {{-- ==================================================== --}}
    @if($showModal)
    <div
            ...
            <div
                ...
                <div>
                    <h3
                        ...
                        <div
                            class="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 flex items-center justify-center">
                            <i class="fas {{ $isEdit ? 'fa-pen' : 'fa-plus' }} text-sm"></i>
                        </div>
                        {{ $isEdit ? 'Edit Data Penjualan' : 'Input Penjualan Manual' }}
                    </h3>
                    <p class="text-[10px] font-bold uppercase tracking-widest text-slate-400 mt-2">Formulir Rangkuman
                        Nota (Tipe A)</p>
                </div>
                <button wire:click="$set('showModal', false)"
                    class="text-slate-400 hover:text-rose-500 transition-colors bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-full w-10 h-10 flex items-center justify-center shadow-sm">
            ...
            <div class="p-8 overflow-y-auto custom-scrollbar">
                <form wire:submit.prevent="store" class="space-y-6 font-jakarta text-xs uppercase">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            <label
                                ...
                                Nota / Dokumen <span class="text-rose-500">*</span></label>
                            <div class="relative">
                                <i
                                    class="fas fa-file-invoice absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>
                                <input type="text" wire:model="no_penjualan"
                                    class="w-full rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-black/40 pl-10 pr-4 py-3.5 font-bold dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm"
                                    placeholder="INV-2026...">
                            </div>
                            @error('no_penjualan') <span class="text-[10px] text-rose-500 font-bold mt-1.5 block"><i
                                    class="fas fa-info-circle mr-1"></i>{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" wire:model="tgl_penjualan"
                                class="w-full rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-black/40 px-4 py-3.5 font-bold dark:text-white cursor-pointer focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm">
                            @error('tgl_penjualan') <span class="text-[10px] text-rose-500 font-bold mt-1.5 block"><i
                                    class="fas fa-info-circle mr-1"></i>{{ $message }}</span> @enderror
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <div>
                            ...
                            <select wire:model="cabang"
                                class="w-full rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#121212] px-4 py-3.5 font-bold dark:text-white cursor-pointer focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm">
                            ...
                            @error('cabang') <span class="text-[10px] text-rose-500 font-bold mt-1.5 block"><i
                                    class="fas fa-info-circle mr-1"></i>{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label
                            ...
                            <div class="relative">
                                ...
                                <input type="text" wire:model="sales_name"
                                    class="w-full rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#121212] pl-10 pr-4 py-3.5 font-bold dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm"
                                    placeholder="Nama Sales...">
                            </div>
                            @error('sales_name') <span class="text-[10px] text-rose-500 font-bold mt-1.5 block"><i
                                    class="fas fa-info-circle mr-1"></i>{{ $message }}</span> @enderror
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <i class="fas fa-store absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>
                            <input type="text" wire:model="nama_pelanggan"
                                class="w-full rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-black/40 pl-10 pr-4 py-3.5 font-bold dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm"
                                placeholder="Nama Toko Tujuan...">
                        </div>
                        @error('nama_pelanggan') <span class="text-[10px] text-rose-500 font-bold mt-1.5 block"><i
                                class="fas fa-info-circle mr-1"></i>{{ $message }}</span> @enderror
                    </div>

                    <div
                        class="bg-emerald-50 dark:bg-emerald-500/10 p-6 rounded-2xl border border-emerald-100 dark:border-emerald-500/20">
                        <label
                            class="block text-[10px] font-black text-emerald-700 dark:text-emerald-400 mb-2 tracking-widest">Total
                            Belanja (Grand Total) <span class="text-rose-500">*</span></label>
                        <div class="relative flex items-center">
                            <span
                                class="absolute left-4 font-black text-emerald-600 dark:text-emerald-400 text-sm">Rp</span>
                            <input type="number" wire:model="total_grand"
                                class="w-full rounded-xl border border-emerald-200 dark:border-emerald-500/30 bg-white dark:bg-black/40 pl-12 pr-4 py-4 font-black text-emerald-700 dark:text-emerald-300 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-lg shadow-sm transition-all"
                                placeholder="0">
                        </div>
                        @error('total_grand') <span class="text-[10px] text-rose-500 font-bold mt-1.5 block"><i
                                class="fas fa-info-circle mr-1"></i>{{ $message }}</span> @enderror
                    </div>

                    <div class="pt-8 flex items-center justify-end gap-3">
                        <button type="button" wire:click="$set('showModal', false)"
                            class="px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/10 transition-colors">
                            Batalkan
                        </button>
                        <button type="submit" wire:loading.attr="disabled"
                            class="px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest text-white bg-blue-600 hover:bg-blue-700 active:scale-95 transition-all shadow-xl shadow-blue-500/30 flex items-center gap-2">
