            ...
            <div class="flex items-center gap-4 w-full xl:w-auto">
                ...
                <div>
                    <h1
                        ...
                        Rekap <span class="text-emerald-500">Penjualan</span>
                    </h1>
                    <p
                        class="text-[9px] font-bold uppercase tracking-[0.3em] opacity-50 mt-1.5 dark:text-slate-400 text-slate-500">
                        Comprehensive Sales Audit ({{ number_format($penjualans->total(), 0, ',', '.') }} Rows)</p>
                </div>
            </div>
            ...
            <div class="flex flex-wrap sm:flex-nowrap gap-2 items-center w-full xl:w-auto justify-end">
                ...
                <div class="relative w-full sm:w-48 group">
                    ...
                    <input wire:model.live.debounce.300ms="search" type="text"
                        ...
                        dark:bg-black/40 dark:border-white/10 dark:text-white bg-slate-100 border-slate-200 shadow-inner" placeholder="Faktur / SKU / Item...">
                </div>
                ...
                <div class="relative w-full sm:w-32" x-data="{ open: false, selected: @entangle('filterCabang').live }">
                    ...
                    <div x-show="open" x-transition class="absolute z-50 mt-2 w-48 border rounded-2xl shadow-2xl p-2 max-h-60 overflow-y-auto custom-scrollbar
                        ...
                        <div @click="selected = []"
                            ...
                            <i class="fas fa-times-circle"></i> Reset Hub
                        </div>
                ...
                <div class="relative w-full sm:w-32" x-data="{ open: false, selected: @entangle('filterSales').live }">
                    ...
                    <div x-show="open" x-transition class="absolute z-50 mt-2 w-48 border rounded-2xl shadow-2xl p-2 max-h-60 overflow-y-auto custom-scrollbar
                        ...
                        <div @click="selected = []"
                            class="px-3 py-2 text-[10px] text-rose-500 font-black uppercase tracking-widest cursor-pointer hover:bg-rose-500/10 rounded-xl mb-1 flex items-center gap-2">
                            <i class="fas fa-times-circle"></i> Reset Sales
                        </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button wire:click="export" wire:loading.attr="disabled" wire:target="export"
                        class="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-600/20 transition-all transform active:scale-95">
                    ...
                    <thead>
                        <tr
                            ...
                            <th
                                ...
                                Cabang</th>
                            <th
                                class="px-4 py-6 border-r dark:border-white/5 border-slate-100 bg-inherit sticky left-[60px] z-30 shadow-xl shadow-black/5 text-emerald-500">
                                No Faktur</th>
                            ...
                            <th class="px-4 py-6 border-r dark:border-white/5 border-slate-100">Period</th>
                            ...
                            <th class="px-4 py-6 border-r dark:border-white/5 border-slate-100">SKU</th>
                            <th class="px-4 py-6 border-r dark:border-white/5 border-slate-100">No Batch</th>
                            <th class="px-4 py-6 border-r dark:border-white/5 border-slate-100">Expired</th>
                            <th class="px-4 py-6 border-r dark:border-white/5 border-slate-100 min-w-[250px]">Nama Item
                            </th>
                            <th
                                class="px-4 py-6 border-r border-emerald-500/20 dark:bg-emerald-600/10 bg-emerald-50 text-emerald-600 text-right">
                                Qty</th>
                            ...
                            <th
                                ...
                                Total Diskon</th>
                            <th class="px-4 py-6 border-r dark:border-white/5 border-slate-100 text-right">Net Sales
                            </th>
                            <th class="px-4 py-6 border-r dark:border-white/5 border-slate-100 text-right">Gross Total
                            </th>
                            <th class="px-4 py-6 border-r dark:border-white/5 border-slate-100 text-right">PPN</th>
                            <th
                                class="px-4 py-6 border-r border-yellow-500/20 dark:bg-yellow-600/10 bg-yellow-50 text-yellow-700 text-right font-black">
                                Total Grand</th>
                            <th
                                class="px-4 py-6 border-r border-blue-500/20 dark:bg-blue-600/10 bg-blue-50 text-blue-600 text-right font-black">
                                Margin</th>
                            <th class="px-4 py-6 border-r dark:border-white/5 border-slate-100">Term</th>
                            <th
                                class="px-4 py-6 border-r border-indigo-500/20 dark:bg-indigo-600/10 bg-indigo-50 text-indigo-700 font-black">
                                Kode Sales</th>
                            ...
                            <th class="px-4 py-6">City Code</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y dark:divide-white/5 divide-slate-100">
                        @forelse($penjualans as $item)
                        <tr class="hover:bg-emerald-500/[0.02] transition-colors group dark:bg-[#0a0a0a] bg-white">
                            <td
                                class="px-4 py-3 border-r dark:border-white/5 border-slate-50 font-black sticky left-0 bg-inherit z-10 text-slate-400">
                                {{ $item->cabang }}</td>
                            <td
                                class="px-4 py-3 border-r dark:border-white/5 border-slate-50 font-mono font-black text-emerald-600 dark:text-emerald-400 sticky left-[60px] bg-inherit z-10 shadow-xl shadow-black/5">
                                {{ $item->trans_no }}</td>

                            <td
                                class="px-4 py-3 border-r dark:border-white/5 border-slate-50 text-[9px] font-bold opacity-60">
                                {{ $item->status }}</td>
                            ...
                            <td class="px-4 py-3 border-r dark:border-white/5 border-slate-50 font-mono opacity-50">
                                {{ $item->kode_item }}</td>
                            <td class="px-4 py-3 border-r dark:border-white/5 border-slate-50 font-mono opacity-80">
                                {{ $item->sku }}</td>
                            ...
                            <td
                                class="px-4 py-3 border-r dark:border-white/5 border-slate-50 text-right font-black text-emerald-500 bg-emerald-500/[0.02]">
                            ...
                            <td
                                class="px-4 py-3 border-r dark:border-white/5 border-slate-50 text-right font-bold text-rose-500 bg-rose-500/[0.02]">
                            ...
                            <td
                                class="px-4 py-3 border-r dark:border-white/5 border-slate-50 text-right font-black text-blue-500 bg-blue-500/[0.02]">
                                {{ number_format($item->margin, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 border-r dark:border-white/5 border-slate-50 opacity-60">
                                {{ $item->pembayaran }}</td>
                            <td
                                class="px-4 py-3 border-r dark:border-white/5 border-slate-50 font-mono font-black text-indigo-500 bg-indigo-500/[0.02]">
                                {{ $item->kode_sales }}</td>
                            <td class="px-4 py-3 border-r dark:border-white/5 border-slate-50 font-bold opacity-80">
                                {{ $item->sales_name }}</td>
                            <td class="px-4 py-3 border-r dark:border-white/5 border-slate-50 text-purple-500 italic">
                                {{ $item->supplier }}</td>
                        ...
                        @empty
                        <tr>
                            <td colspan="37" class="px-6 py-32 text-center opacity-20">
                                <i class="fas fa-file-invoice text-8xl mb-4"></i>
