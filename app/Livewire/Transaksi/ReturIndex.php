...
class ReturIndex extends Component
{
    ...
    public function edit($id) {
        ...
        $this->no_dokumen = $data->no_retur ?? $data->trans_no; 
        $this->tanggal = $data->tgl_retur;
    ...
    public function store() {
        ...
        $dbData = [
            'no_retur' => $this->no_dokumen, 'tgl_retur' => $this->tanggal, 'cabang' => $this->cabang,
            'sales_name' => $this->sales_name, 'nama_pelanggan' => $this->nama_pelanggan, 'total_grand' => $this->nominal,
        ];
        if ($this->isEdit) { Retur::findOrFail($this->record_id)->update($dbData); session()->flash('message', 'Data diperbarui!'); } 
        else { Retur::create($dbData); session()->flash('message', 'Data baru ditambahkan!'); }
        $this->showModal = false;
    }

    public function delete($id) {
        if(Auth::user()->role === 'operator') return;
        Retur::findOrFail($id)->delete(); session()->flash('message', 'Data dihapus!');
    }
    ...
    public function importData() {
        ...
        // $importService = new \App\Services\Import\ReturImportService();
        // $importService->import($this->file_excel->getRealPath());
        session()->flash('message', 'Data Retur dari Excel berhasil diimpor!');
    ...
    public function render() {
        ...
        if (!empty($this->search)) {
            $query->where(function($q) {
                $q->where('no_retur', 'like', '%'.$this->search.'%')->orWhere('nama_pelanggan', 'like', '%'.$this->search.'%')->orWhere('sales_name', 'like', '%'.$this->search.'%');
            });
        }
        if (!empty($this->filter_cabang)) { $query->where('cabang', $this->filter_cabang); }
        if (!empty($this->start_date) && !empty($this->end_date)) { $query->whereBetween('tgl_retur', [$this->start_date, $this->end_date]); } 
        elseif (!empty($this->start_date)) { $query->whereDate('tgl_retur', '>=', $this->start_date); } 
        elseif (!empty($this->end_date)) { $query->whereDate('tgl_retur', '<=', $this->end_date); }

        $dataTabel = $query->orderBy('tgl_retur', 'desc')->paginate(15);
        return view('livewire.transaksi.retur-index', compact('dataTabel'))->layout('layouts.app', ['header' => 'Data Retur']);
    }
}